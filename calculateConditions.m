...
% 
% trialIndex: first column is number of moving types , second column is 
%               condition number in each moving type
% 
% March 2019 by BYC
global TRIALINFO
...
if TRIALINFO.motionType(2)
    ...
    mov2MdHdHsFdFs = [sort(repmat(TRIALINFO.moveDuration',length(TRIALNFO.headingDegree)*length(TRIALINFO.fixSpeed)*length(TRIALINFO.fixMoveDirection)*length(TRIALINFO.headingSpeed),1),1),...
...
if TRIALINFO.motionType(3)
   ...
   mov3HdHsRdRs = [sort(repmat(TRIALINFO.headingDegree',length(TRLAINFO.headingSpeed)*length(TRIALINFO.rotationSpeed)*length(TRIALINFO.rotationDegree),1),1),...
       repmat(mov3HsRdRs,length(TRIALINFO.headingDegree),1)];
   
   mov3MdHdHsRdRs = [sort(repmat(TRILAINFO.moveDuration',length(TRIALINFO.headingDegree)*length(TRLAINFO.headingSpeed)*length(TRIALINFO.rotationSpeed)*length(TRIALINFO.rotationDegree),1),1),...
